...
def to_yaml(rows: list[dict]) -> str:
    ...
    for r in rows:
        ...
        # Our robot is slower than Nova Carter (0.6 vs 1.5 m/s cap), so DynaNav's
        # timeout would cut episodes off mid-approach. Budget on distance at our
        # own speed, with headroom for the policy's non-straight path, and never
        # less than DynaNav allowed.
